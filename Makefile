...
bash-scripts-configure:
	...
	chmod +x ${PROJECT_ROOT_PATH}/${BASH_ENCRYPT_BASH_DIR_PATH}/*/**
	chmod +x ${PROJECT_ROOT_PATH}/${BASH_ENCRYPT_HOOKS_DIR_PATH}/*/**

# configure gits precommit hook
# it must be renamed to pre-commit in order for git to use it.
git-configure-hooks:
	. ./${BASH_ENCRYPT_BASH_DIR_PATH}/git-configure-hooks.sh

# encrypt a single secret filee - example usage:
# make secret-encrypt file="src/test/text.secret"
secret-encrypt:
	. ${BASH_ENCRYPT_BASH_DIR_PATH}/encrypt-file.sh $(ARGS)

# decrypt a single secret file - example usage:
# make secret-decrypt file="src/test/text.secret"
secret-decrypt:
	. ${BASH_ENCRYPT_BASH_DIR_PATH}/decrypt-file.sh $(ARGS)

# encrypt all secret files
secrets-encrypt-all:
	. ${BASH_ENCRYPT_BASH_DIR_PATH}/encrypt-all.sh
	
# decrypt all secret files
secrets-decrypt-all:
	. ${BASH_ENCRYPT_BASH_DIR_PATH}/decrypt-all.sh

# check all secret files are encrypted
secrets-check-all:
	. ${BASH_ENCRYPT_BASH_DIR_PATH}/check-all.sh

# run to make sure the project paths are set correctly
bash-encrypt-testing:
	. ./${BASH_ENCRYPT_ROOT_DIR}/testing.sh
